    ...
    public partial class Delta
    {
        ...
        private void DeltaCluster(FbConnection conn, List<DeltaReport> list)
        {
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._comparisonSet.Uid, "CLUSTER", (string)dr["cluster_name"], Strings.ObjectInSource);
                    list.Add(report);
                }
            }
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._comparisonSet.Uid, "CLUSTER", (string)dr["cluster_name"], Strings.ObjectInTarget);
                    list.Add(report);
                }
            }
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var sourceCluster = new Cluster
                    {
                        ...
                        Hashkeys = dr["src_hashkeys"] is DBNull ? null : (decimal?)dr["src_hashkeys"],
                    ...
                    var targetCluster = new Cluster
                    {
                        ...
                        Hashkeys = dr["tgt_hashkeys"] is DBNull ? null : (decimal?)dr["tgt_hashkeys"],
