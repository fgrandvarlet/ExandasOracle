		...
		private void DeltaClusterColumnMapping(FbConnection conn, List<DeltaReport> list)
		{
			...
			using (FbDataReader dr = cmd.ExecuteReader())
			{
				while (dr.Read())
				{
					...
					var report = new DeltaReport(this._comparisonSet.Uid, ENTITY, objectValue, (string)dr["cluster_name"], Strings.ObjectInSource);
					list.Add(report);
				}
			}
			...
			using (FbDataReader dr = cmd.ExecuteReader())
			{
				...
				{
					var objectValue = string.Format("{0}->{1}", (string)dr["clu_column_name"], (string)dr["table_name"]);
					var report = new DeltaReport(this._comparisonSet.Uid, ENTITY, objectValue, (string)dr["cluster_name"], Strings.ObjectInTarget);
					list.Add(report);
				}
			}
