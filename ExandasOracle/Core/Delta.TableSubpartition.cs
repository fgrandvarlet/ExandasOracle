        ...
        private void DeltaTableSubpartition(FbConnection conn, List<DeltaReport> list)
        {
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    ...
                    var report = new DeltaReport(this._comparisonSet.Uid, "TABLE SUBPARTITION", (string)dr["subpartition_name"], parentObject, Strings.ObjectInSource);
                    list.Add(report);
                }
            }
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                ...
                {
                    var parentObject = string.Format("{0}.{1}", (string)dr["table_name"], (string)dr["partition_name"]);
                    var report = new DeltaReport(this._comparisonSet.Uid, "TABLE SUBPARTITION", (string)dr["subpartition_name"], parentObject, Strings.ObjectInTarget);
                    list.Add(report);
                }
            }
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var sourceTableSubpartition = new TableSubpartition
                    {
                        ...
                        SubpartitionPosition = dr["src_subpartition_position"] is DBNull ? null : (decimal?)dr["src_subpartition_position"],
                    ...
                    var targetTableSubpartition = new TableSubpartition
                    {
                        ...
                        SubpartitionPosition = dr["tgt_subpartition_position"] is DBNull ? null : (decimal?)dr["tgt_subpartition_position"],
