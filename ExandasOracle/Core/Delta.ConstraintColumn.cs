        ...
        private void DeltaConstraintColumn(FbConnection conn, List<DeltaReport> list)
        {
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    ...
                    var report = new DeltaReport(this._comparisonSet.Uid, "CONSTRAINT COLUMN", (string)dr["column_name"], parentObject, LabelId.ObjectInSourceNotInTarget);
                    list.Add(report);
                }
            }
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                ...
                {
                    var parentObject = string.Format("{0}.{1}", (string)dr["table_name"], (string)dr["constraint_name"]);
                    var report = new DeltaReport(this._comparisonSet.Uid, "CONSTRAINT COLUMN", (string)dr["column_name"], parentObject, LabelId.ObjectInTargetNotInSource);
                    list.Add(report);
                }
            }
