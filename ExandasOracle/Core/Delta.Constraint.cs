...
namespace ExandasOracle.Core
{
    public partial class Delta
    {
        ...
        private void DeltaConstraint(FbConnection conn, List<DeltaReport> list)
        {
            string sql;
            FbCommand cmd;

            // phase 1 : source minus target
            sql = "SELECT s.table_name, s.constraint_name FROM src_constraints s" +
                " LEFT JOIN tgt_constraints t USING(table_name, constraint_name)" +
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._comparisonSet.Uid, "CONSTRAINT", (string)dr["constraint_name"], (string)dr["table_name"], LabelId.ObjectInSourceNotInTarget);
                    list.Add(report);
                }
            }

            // phase 2 : target minus source
            sql = "SELECT t.table_name, t.constraint_name FROM tgt_constraints t" +
                " LEFT JOIN src_constraints s USING(table_name, constraint_name)" +
                " JOIN common_tables USING(table_name)" +
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._comparisonSet.Uid, "CONSTRAINT", (string)dr["constraint_name"], (string)dr["table_name"], LabelId.ObjectInTargetNotInSource);
                    list.Add(report);
                }
            }

            // phase 3 : property differences between source and target
            sql = "SELECT * FROM comp_constraints";
            cmd = new FbCommand(sql, conn);
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var sourceConstraint = new Constraint
                    {
                        ConstraintName = (string)dr["constraint_name"],
                        ConstraintType = dr["src_constraint_type"] is DBNull ? null : (string)dr["src_constraint_type"],
                    ...
                    var targetConstraint = new Constraint
                    {
                        ConstraintName = (string)dr["constraint_name"],
                        ConstraintType = dr["tgt_constraint_type"] is DBNull ? null : (string)dr["tgt_constraint_type"],
                        TableName = (string)dr["table_name"],
                    ...
                    sourceConstraint.Compare(targetConstraint, this._comparisonSet.Uid, list);
                }
            }
        }
