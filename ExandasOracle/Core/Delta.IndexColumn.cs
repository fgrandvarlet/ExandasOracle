        ...
        private void DeltaIndexColumn(FbConnection conn, List<DeltaReport> list)
        {
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    ...
                    var report = new DeltaReport(this._comparisonSet.Uid, "INDEX COLUMN", (string)dr["column_name"], parentObject, Strings.ObjectInSource);
                    list.Add(report);
                }
            }
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                ...
                {
                    var parentObject = string.Format("{0}.{1}", (string)dr["table_name"], (string)dr["index_name"]);
                    var report = new DeltaReport(this._comparisonSet.Uid, "INDEX COLUMN", (string)dr["column_name"], parentObject, Strings.ObjectInTarget);
                    list.Add(report);
                }
            }
