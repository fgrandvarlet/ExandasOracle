    ...
    public partial class Delta
    {
		...
		private void DeltaSequence(FbConnection conn, List<DeltaReport> list)
		{
			...
			using (FbDataReader dr = cmd.ExecuteReader())
			{
				while (dr.Read())
				{
					var report = new DeltaReport(this._comparisonSet.Uid, "SEQUENCE", (string)dr["sequence_name"], LabelId.ObjectInSourceNotInTarget);
					list.Add(report);
				}
			}
			...
			using (FbDataReader dr = cmd.ExecuteReader())
			{
				while (dr.Read())
				{
					var report = new DeltaReport(this._comparisonSet.Uid, "SEQUENCE", (string)dr["sequence_name"], LabelId.ObjectInTargetNotInSource);
					list.Add(report);
				}
			}
