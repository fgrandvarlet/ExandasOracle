        ...
        private void DeltaIndexSubpartition(FbConnection conn, List<DeltaReport> list)
        {
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    ...
                    var report = new DeltaReport(this._comparisonSet.Uid, "INDEX SUBPARTITION", (string)dr["subpartition_name"], parentObject, LabelId.ObjectInSourceNotInTarget);
                    list.Add(report);
                }
            }
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                ...
                {
                    var parentObject = string.Format("{0}.{1}", (string)dr["index_name"], (string)dr["partition_name"]);
                    var report = new DeltaReport(this._comparisonSet.Uid, "INDEX SUBPARTITION", (string)dr["subpartition_name"], parentObject, LabelId.ObjectInTargetNotInSource);
                    list.Add(report);
                }
            }
