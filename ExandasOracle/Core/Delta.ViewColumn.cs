...
namespace ExandasOracle.Core
{
    public partial class Delta
    {
		...
		private void DeltaViewColumn(FbConnection conn, List<DeltaReport> list)
		{
			...
			sql = "SELECT s.table_name, s.column_name FROM src_view_cols s" +
				" LEFT JOIN tgt_view_cols t USING (table_name, column_name)" +
				" JOIN common_views cv ON s.table_name = cv.view_name" +
			...
			sql = "SELECT t.table_name, t.column_name FROM tgt_view_cols t" +
				" LEFT JOIN src_view_cols s USING (table_name, column_name)" +
				" JOIN common_views cv ON t.table_name = cv.view_name" +
			...
			sql = "SELECT * FROM comp_view_cols";
			cmd = new FbCommand(sql, conn);
			...
			using (FbDataReader dr = cmd.ExecuteReader())
			{
				while (dr.Read())
				{
					var sourceViewColumn = new ViewColumn
					{
					...
					var targetViewColumn = new ViewColumn
					{
						TableName = (string)dr["table_name"],
						ColumnName = (string)dr["column_name"],
					...
					sourceViewColumn.Compare(targetViewColumn, this._comparisonSet.Uid, list);
				}
			}
		}
