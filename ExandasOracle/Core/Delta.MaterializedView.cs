    ...
    public partial class Delta
    {
        ...
        private void DeltaMaterializedView(FbConnection conn, List<DeltaReport> list)
        {
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._comparisonSet.Uid, "MATERIALIZED VIEW", (string)dr["mview_name"], LabelId.ObjectInSourceNotInTarget);
                    list.Add(report);
                }
            }
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._comparisonSet.Uid, "MATERIALIZED VIEW", (string)dr["mview_name"], LabelId.ObjectInTargetNotInSource);
                    list.Add(report);
                }
            }
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var sourceMaterializedView = new MaterializedView
                    {
                        ...
                        QueryLen = dr["src_query_len"] is DBNull ? null : (int?)dr["src_query_len"],
                    ...
                    var targetMaterializedView = new MaterializedView
                    {
                        ...
                        QueryLen = dr["tgt_query_len"] is DBNull ? null : (int?)dr["tgt_query_len"],
