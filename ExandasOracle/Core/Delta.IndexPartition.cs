    ...
    public partial class Delta
    {
        ...
        private void DeltaIndexPartition(FbConnection conn, List<DeltaReport> list)
        {
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._comparisonSet.Uid, "INDEX PARTITION", (string)dr["partition_name"], (string)dr["index_name"], LabelId.ObjectInSourceNotInTarget);
                    list.Add(report);
                }
            }
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._comparisonSet.Uid, "INDEX PARTITION", (string)dr["partition_name"], (string)dr["index_name"], LabelId.ObjectInTargetNotInSource);
                    list.Add(report);
                }
            }
