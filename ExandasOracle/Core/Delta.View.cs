    ...
    public partial class Delta
    {
        ...
        private void DeltaView(FbConnection conn, List<DeltaReport> list)
        {
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._comparisonSet.Uid, "VIEW", (string)dr["view_name"], LabelId.ObjectInSourceNotInTarget);
                    list.Add(report);
                }
            }
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._comparisonSet.Uid, "VIEW", (string)dr["view_name"], LabelId.ObjectInTargetNotInSource);
                    list.Add(report);
                }
            }
