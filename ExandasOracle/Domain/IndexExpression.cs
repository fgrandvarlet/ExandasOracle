        ...
        public void Compare(IndexExpression target, ComparisonSet comparisonSet, List<DeltaReport> list)
        {
            ...
            if (this.TableOwner != target.TableOwner && comparisonSet.Schema1 == comparisonSet.Schema2)
            {
                list.Add(new DeltaReport(
                    comparisonSet.Uid, ENTITY, objectValue, null, Strings.PropertyDifference, "TABLE_OWNER", this.TableOwner, target.TableOwner
                    ));
            }
            if (this.TableName != target.TableName)
            {
                list.Add(new DeltaReport(
                    comparisonSet.Uid, ENTITY, objectValue, null, Strings.PropertyDifference, "TABLE_NAME", this.TableName, target.TableName
                    ));
            }
            if (this.ColumnExpression != target.ColumnExpression)
            {
                list.Add(new DeltaReport(
                    comparisonSet.Uid, ENTITY, objectValue, null, Strings.PropertyDifference, "COLUMN_EXPRESSION", this.ColumnExpression, target.ColumnExpression
                    ));
            }
        }
